  ...
  try {
    ...
    switch (action) {
      case 'generateTopicProposals': {
        ...
        if (!Array.isArray(interests) || !Array.isArray(topicsToAvoid) || !learningLanguage || !level) {
          return res.status(400).json({ error: 'Invalid payload for generateTopicProposals' });
        }
        ...
        try {
            ...
            console.error("Failed to parse Gemini response for proposals:", response.text);
      ...
      case 'generateArticleStream': {
        ...
        if (!topic || !settings || !settings.learningLanguage || !settings.level) {
            return res.status(400).json({ error: 'Invalid payload for generateArticleStream' });
        }
      ...
      case 'translateWord': {
        ...
        if (!word || !from || !to) {
            return res.status(400).json({ error: 'Invalid payload for translateWord' });
        }
      ...
      case 'generateQuizQuestion': {
        const { articleContent, learningLanguage, level, levelDescription } = payload;
         if (!articleContent || !learningLanguage || !level) {
            return res.status(400).json({ error: 'Invalid payload for generateQuizQuestion' });
        }
      ...
      case 'evaluateQuizAnswer': {
        ...
        if (!articleContent || !question || !userAnswer || !learningLanguage || !level) {
            return res.status(400).json({ error: 'Invalid payload for evaluateQuizAnswer' });
        }
